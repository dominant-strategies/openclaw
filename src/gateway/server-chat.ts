...
export function createAgentEventHandler({
  ...
  const emitChatDelta = (
    ...
    text: string,
    delta?: unknown,
    opts?: { force?: boolean; textAlreadyVisible?: boolean },
  ) => {
    ...
    if (opts?.textAlreadyVisible) {
      if (cleanedText !== buffer.text) {
        buffer.text = cleanedText;
      }
    } else {
      ...
      if (isSuppressedControlReplyLeadFragment(mergedRawText)) {
        buffer.text = mergedRawText;
        return;
      }
      buffer.text = startsWithSilentToken(mergedRawText, SILENT_REPLY_TOKEN)
  ...
  const resolveBufferedChatTextState = (clientRunId: string, sourceRunId: string) => {
    ...
    const rawBufferText = chatRunState.rawBuffers.get(clientRunId) ?? "";
    ...
    const shouldSuppressSilent =
      ...
      isSuppressedControlReplyText(rawBufferText) ||
