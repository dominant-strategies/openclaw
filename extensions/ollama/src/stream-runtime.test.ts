...
describe("buildOllamaChatRequest", () => {
  ...
  it("serializes top-level think when provided", () => {
    expect(
      buildOllamaChatRequest({
        modelId: "qwen3.5:9b",
        messages: [{ role: "user", content: "hello" }],
...
describe("createOllamaStreamFn streaming events", () => {
  it("emits start, text_start, text_delta, text_end, done for text responses", async () => {
    await withMockNdjsonFetch(
      ...
      async () => {
        ...
        expect(textStartEvent?.partial.content).toEqual([{ type: "text", text: "" }]);
  ...
  it("emits start, toolcall_start, toolcall_end, done for tool-call-only responses", async () => {
    await withMockNdjsonFetch(
      [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"","tool_calls":[{"function":{"name":"bash","arguments":{"command":"ls"}}}]},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":10,"eval_count":5}',
  ...
  it("emits text and tool-call events before done for mixed text + tool responses", async () => {
    await withMockNdjsonFetch(
      [
  ...
  it("emits text_end as soon as Ollama switches from text to tool calls", async () => {
    ...
    try {
      ...
      expect(toolCallStartEvent).toMatchObject({
  ...
  it("deduplicates repeated streamed tool-call updates into one final tool call", async () => {
    await withMockNdjsonFetch(
      [
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"","tool_calls":[{"function":{"index":0,"name":"bash","arguments":{"command":"l"}}}]},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":"","tool_calls":[{"function":{"index":0,"name":"bash","arguments":{"command":"ls -la"}}}]},"done":false}',
        '{"model":"m","created_at":"t","message":{"role":"assistant","content":""},"done":true,"prompt_eval_count":10,"eval_count":5}',
      ],
      async () => {
        const stream = await createOllamaTestStream({ baseUrl: "http://ollama-host:11434" });
        ...
        const doneEvent = events.at(-1) as
          ...
          | undefined;
        const finalToolCalls = doneEvent?.message.content.filter(
  ...
  it("serializes top-level think=false when thinking is off", async () => {
    await withMockNdjsonFetch(
      ...
      async (fetchMock) => {
        ...
        const streamFn = createConfiguredOllamaCompatStreamWrapper({
          provider: "ollama",
          modelId: "qwen3.5:9b",
          model,
  ...
  it("keeps tool call ids stable between streamed events and the final message", async () => {
    await withMockNdjsonFetch(
      ...
      async () => {
        ...
        const streamedToolEnd = events.find((event) => event.type === "toolcall_end") as
          | { type: "toolcall_end"; toolCall: { id: string; name: string } }
          | undefined;
        const doneEvent = events.at(-1) as
