...
function readAppendedLeafPathMessages(params: { sessionFile: string; offset: number }): {
...
export class LcmContextEngine implements ContextEngine {
  ...
  private async interceptLargeToolResults(params: {
    ...
    for (const item of params.message.content) {
      ...
      const isPlainTextToolResult = rawType === "text" && typeof record.text === "string";
      ...
      interceptedAny = true;
      const toolName = safeString(record.name) ?? topLevelToolName ?? "tool-result";
      const externalized = await this.externalizeLargeTextPayload({
        conversationId: params.conversationId,
  ...
  async prepareSubagentSpawn(params: {
    ...
    const childTokenCap = parentGrant
      ? Math.min(
          getRuntimeExpansionAuthManager().getRemainingTokenBudget(parentGrantId!) ??
