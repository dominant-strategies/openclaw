...
| `LCM_EXPANSION_MODEL`             | _(from OpenClaw)_    | Model override for `lcm_expand_query` sub-agent (e.g. `anthropic/claude-haiku-4-5`)                                         |
| `LCM_EXPANSION_PROVIDER`          | _(from OpenClaw)_    | Provider override for `lcm_expand_query` sub-agent                                                                          |
...
```json
{
  "plugins": {
    "entries": {
      "lossless-claw": {
        "config": {
          "statelessSessionPatterns": ["agent:*:subagent:**", "agent:ops:subagent:**"],
