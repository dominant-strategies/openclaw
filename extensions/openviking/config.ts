...
function resolveDefaultBaseUrl(): string {
  return process.env.OPENVIKING_BASE_URL || process.env.OPENVIKING_URL || DEFAULT_BASE_URL;
}
...
const jsonSchema = {
  ...
  properties: {
    ...
    captureMaxLength: { type: "number" },
    autoRecall: { type: "boolean" },
    recallLimit: { type: "number" },
    recallScoreThreshold: { type: "number" },
    ingestReplyAssist: { type: "boolean" },
    ingestReplyAssistMinSpeakerTurns: { type: "number" },
    ingestReplyAssistMinChars: { type: "number" },
  },
};
...
export const memoryOpenVikingConfigSchema = {
  parse(value: unknown): Required<MemoryOpenVikingConfig> {
    const cfg =
      value && typeof value === "object" && !Array.isArray(value)
        ...
        : {};
    assertAllowedKeys(cfg, Object.keys(jsonSchema.properties), "openviking config");
    ...
    return {
      ...
      ingestReplyAssist:
        typeof cfg.ingestReplyAssist === "boolean"
          ? cfg.ingestReplyAssist
          : DEFAULT_INGEST_REPLY_ASSIST,
      ...
      ingestReplyAssistMinChars: Math.max(
        ...
        Math.min(
          10_000,
          Math.floor(
            toNumber(cfg.ingestReplyAssistMinChars, DEFAULT_INGEST_REPLY_ASSIST_MIN_CHARS),
          ),
        ),
      ),
    };
  ...
  uiHints: {
    mode: {
      label: "Mode",
      help: "local = plugin starts OpenViking; remote = use an existing HTTP server",
    },
    configPath: {
    ...
    agentId: {
      label: "Agent ID",
      placeholder: "default",
      help: "Identifies this agent to OpenViking",
    },
    apiKey: {
    ...
    ingestReplyAssist: {
      ...
      help: "Add a lightweight reply instruction when transcript-like ingestion is detected",
      advanced: true,
    },
