...
export function getCaptureDecision(
  ...
  if (COMMAND_TEXT_RE.test(normalizedText)) {
    return { shouldCapture: false, reason: "command_text", normalizedText };
  }
  if (NON_CONTENT_TEXT_RE.test(normalizedText)) {
    return { shouldCapture: false, reason: "non_content_text", normalizedText };
  }
  if (SUBAGENT_CONTEXT_RE.test(normalizedText)) {
    return { shouldCapture: false, reason: "subagent_context", normalizedText };
  }
  if (looksLikeQuestionOnlyText(normalizedText)) {
    return { shouldCapture: false, reason: "question_text", normalizedText };
  }
  ...
  for (let index = startIndex; index < messages.length; index += 1) {
    ...
    if (typeof content === "string" && content.trim()) {
      ...
      for (const block of content) {
        const blockObj = block as Record<string, unknown>;
        if (blockObj?.type === "text" && typeof blockObj.text === "string") {
          texts.push(`[${role}]: ${blockObj.text.trim()}`);
        }
      }
    }
  }
...
export function extractLatestUserText(messages: unknown[] | undefined): string {
  ...
  for (let index = texts.length - 1; index >= 0; index -= 1) {
    const normalized = sanitizeUserTextForCapture(texts[index] ?? "");
    if (normalized) {
      return normalized;
    }
  }
