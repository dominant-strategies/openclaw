...
import { trimForLog, toJsonLog, summarizeExtractedMemories } from "./memory-ranking.js";
import { getCaptureDecision, extractNewTurnTexts } from "./text-utils.js";
...
async function tryLegacyCompact(params: {
  ...
  const candidates = ["openclaw/context-engine/legacy", "openclaw/dist/context-engine/legacy.js"];
...
export function createMemoryOpenVikingContextEngine(params: {
  ...
  const { id, name, version, cfg, logger, getClient, resolveAgentId } = params;
  ...
  return {
      ...
      try {
        ...
        try {
          ...
          if (extracted.length === 0) {
            warnOrInfo(
              logger,
              "openviking: auto-capture completed but extract returned 0 memories. Check OpenViking server logs.",
            );
          }
        } finally {
    ...
    async compact(compactParams): Promise<CompactResult> {
      ...
      warnOrInfo(logger, "openviking: legacy compaction delegation unavailable; skipping compact");
      return {
        ok: true,
        compacted: false,
