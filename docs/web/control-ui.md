...
- `chat.send` responses and `chat` events may include `selection` metadata so clients can show the
  effective run model separately from the session default model.
